...
export function parseCompactionV2State(summary?: string): CompactionV2State | null {
  ...
  try {
    ...
    if (parsed?.version !== "v2" || !Array.isArray(parsed.chain)) return null;
    return parsed;
  } catch {
    return null;
  }
...
function trimSummaryByTokens(text: string, maxNodeTokens: number): string {
  ...
  if (est <= maxNodeTokens) return text;
...
function mergeOldest(nodes: CompactionV2Node[]): CompactionV2Node[] {
  ...
  const merged: CompactionV2Node = {
    ...
    artifactIndex: {
      ...
      commands: [...new Set([...(a.artifactIndex.commands ?? []), ...(b.artifactIndex.commands ?? [])])],
...
export function appendNodeWithLimits(params: {
  ...
  let chain = [...priorChain, { ...params.node, anchorId, summary: trimSummaryByTokens(params.node.summary, params.limits.maxNodeTokens) }];

  while (chain.length > params.limits.maxSummaryNodes || totalChainTokens(chain) > params.limits.maxChainTokens) {
    chain = mergeOldest(chain);
  }
...
export function renderCompactionV2Summary(state: CompactionV2State): string {
  const human = [
    ...
    ...(state.chain.length > 0
      ? [
          "",
          ...state.chain.map((node, idx) => `### ${idx + 1}. ${node.anchorId} (${node.fromEntryId} → ${node.toEntryId})\n${node.summary}`),
        ]
      : []),
  ].join("\n");
...
export function resolveDeltaMessages<T extends { timestamp?: number }>(
  ...
  if (priorToEntryId) {
    ...
    if (idx >= 0 && idx + 1 < messages.length) start = idx + 1;
  }

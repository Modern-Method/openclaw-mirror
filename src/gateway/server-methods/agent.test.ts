...
describe("gateway agent handler", () => {
  ...
  it("forwards the current task contract to agentCommand ingress", async () => {
    primeMainAgentRun();

    await invokeAgent(
      {
        message: "work the pinned task",
        sessionKey: "agent:main:main",
        lane: "pinned",
        currentTaskId: "task-42",
        idempotencyKey: "test-current-task-contract",
      },
      { reqId: "task-contract-1" },
    );

    await vi.waitFor(() => expect(mocks.agentCommand).toHaveBeenCalled());
    const callArgs = mocks.agentCommand.mock.calls.at(-1)?.[0] as
      | { lane?: string; currentTaskId?: string }
      | undefined;
  ...
  it("handles bare /new by resetting the same session and sending reset greeting prompt", async () => {
    ...
    expect(mocks.performGatewaySessionReset).toHaveBeenCalledTimes(1);
    const call = readLastAgentCommandCall();
    // Message is dynamically built with current date/time and startup guidance.
    expect(call?.message).toContain("A new session was started via /new or /reset");
    expect(call?.message).toContain("Session Startup sequence");
